...
SELECT KQT.MAMH, KQT.KQUA, COUNT(KQT.MAHV)
FROM KETQUATHI KQT 
...
FROM MONHOC ,
(
    SELECT KETQUATHI.MAMH--, COUNT(DISTINCT MAHV)
    FROM KETQUATHI
    WHERE LANTHI = 1 AND KQUA = 'Khong Dat'
    GROUP BY KETQUATHI.MAMH
    HAVING COUNT(DISTINCT MAHV) > 1
) AS ABC
WHERE MONHOC.MAMH = ABC.MAMH
...
WHERE HOCVIEN.MAHV = K1.MAHV
AND NOT EXISTS (
    SELECT *
    FROM KETQUATHI K2
    WHERE K2.MAHV = HOCVIEN.MAHV AND K2.LANTHI = (
        SELECT MAX(K3.LANTHI) FROM KETQUATHI K3 WHERE K3.MAHV = HOCVIEN.MAHV GROUP BY K3.MAHV
    ) 
...
CREATE VIEW MH_MAX_DIEM AS (
    ...
    FROM KETQUATHI KQ
    WHERE KQ.LANTHI = (
        SELECT MAX(KQ1.LANTHI) FROM KETQUATHI KQ1 WHERE KQ.MAHV = KQ1.MAHV AND KQ.MAMH = KQ1.MAMH GROUP BY KQ1.MAHV
    )
